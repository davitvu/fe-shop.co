                        ...
                        <div className='flex gap-5'>
                            {headerActions.map((item) => (
                                item.badgeCount ? (
                                    ...
                                    <div className='dropdown dropdown-end' key={item.id} onClick={item.id === 'user' && !isAuthenticated ? () => navigate('/login') : () => { }}>
                                        <div tabIndex={item.id === 'user' ? 0 : -1} className='cursor-pointer ' >
                                            {item.icon}
                                        </div>
                                        {(isAuthenticated) && (
                                            <div tabIndex={-1} className="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm absolute right-0 top-11">
                                                {userOptions.map((o) => {
                                                    if (o.href) {
                                                        return <Link key={o.id} to={o.href} className='block bg-white py-2 px-3 hover:bg-[#f0f0f0]'>{o.label}</Link>
                                                    }
                                                    if (o.onClick) {
                                                        return <p key={o.id} onClick={o.id === 'logout' ? logout : o.onClick} className='btn bg-white py-2 px-3 hover:bg-[#f0f0f0] cursor-pointer'>{o.label}</p>
                                                    }
                                                })}
                                            </div>
                                        )}
